...
Multi-agent AI SDLC Copilot — requirement documents in, Jira-ready tasks/tests/sprint plans/API specs/architecture/compliance out, via 27 specialised LLM agents with LangGraph conditional routing and a Streamlit demo UI.
...
pytest                                              # 70 tests, no real API calls
ruff check .
mypy sdlc_copilot
uvicorn sdlc_copilot.main:app --reload              # http://localhost:8000/docs
USE_LANGGRAPH=true uvicorn sdlc_copilot.main:app --reload  # LangGraph routing
streamlit run streamlit_app/app.py                  # demo UI
sdlc-copilot agents
...
sdlc_copilot/
  ...
  services/
    pipeline.py    # SDLCPipelineService.run() / .stream() — ingest→chunk→embed→orchestrate→persist
  ingestion/
...
tests/             # 70 tests; conftest.py has mock_llm, low_confidence_llm, sample_state fixtures
```
...
- **Streamlit**: calls `SDLCPipelineService` in-process (no HTTP); `2_Progress.py` consumes `service.stream()`; confidence badge 🟢 ≥0.8 / 🟡 ≥0.6 / 🔴 <0.6.
